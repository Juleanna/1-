...
Процедура ЗаполнитьПоТехкарточкамНаСервере()
    ...
    Сообщить("Документ заполнен по техкарточкам");
...
Процедура ЗаписатьДвиженияПоРегистру(Отказ)
    ...
    Попытка
        ...
        Сообщить("=== ВСЕ ДВИЖЕНИЯ УСПЕШНО ЗАПИСАНЫ В ТоварыНаСкладах ===");

        // ТЕПЕРЬ ЗАПИСЫВАЕМ В РЕГИСТР СвободныеОстатки
        ЗаписатьВСвободныеОстатки(ТаблицаСписания, ОрганизацияДляЗаписи, Отказ);
...
Процедура ЗаписатьВСвободныеОстатки(ТаблицаСписания, ОрганизацияДляЗаписи, Отказ)

    Сообщить(">>> ЗАПИСЬ В РЕГИСТР СвободныеОстатки <<<");

    Попытка
        // Создаем набор записей для регистра СвободныеОстатки
        НаборСвободныхОстатков = РегистрыНакопления.СвободныеОстатки.СоздатьНаборЗаписей();
        НаборСвободныхОстатков.Отбор.Регистратор.Установить(Ссылка);

        // 1. Списание ингредиентов (расход)
        Сообщить("=== РАСХОД ИНГРЕДИЕНТОВ (СвободныеОстатки) ===");
        Для Каждого СтрокаСписания Из ТаблицаСписания Цикл
            НоваяЗапись = НаборСвободныхОстатков.Добавить();
            НоваяЗапись.Период = Дата;
            НоваяЗапись.Регистратор = Ссылка;
            НоваяЗапись.ВидДвижения = ВидДвиженияНакопления.Расход;
            НоваяЗапись.Номенклатура = СтрокаСписания.Ингредиент;
        ...
        // 2. Оприходование готовых блюд (приход)
        ...
        Сообщить("Всего движений в СвободныеОстатки: " + Формат(НаборСвободныхОстатков.Количество(), "ЧГ=0"));
        НаборСвободныхОстатков.Записать();
        Сообщить("=== ДВИЖЕНИЯ В СвободныеОстатки УСПЕШНО ЗАПИСАНЫ ===");

    Исключение
        Сообщить("ОШИБКА при записи в СвободныеОстатки: " + ОписаниеОшибки());
        Отказ = Истина;
    КонецПопытки;
...
Функция ПолучитьОстатокТовараНаСкладе(Номенклатура, Склад)
    ...
    Попытка
        ...
        |ИЗ
        |    РегистрНакопления.СвободныеОстатки.Остатки(, Номенклатура = &Номенклатура И Склад = &Склад) КАК СвободныеОстаткиОстатки";
...
Функция НайтиИмяРегистраНакопления()
    ...
    // Пробуем найти регистр ТоварыНаСкладах
    Попытка
        Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1 * ИЗ РегистрНакопления.ТоварыНаСкладах");
        Запрос.Выполнить();
