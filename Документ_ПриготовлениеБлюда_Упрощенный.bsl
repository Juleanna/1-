...
Функция ПолучитьОстатокТовараНаСкладе(Номенклатура, Склад)
    ...
    Попытка
        ...
        МассивВариантов.Добавить("РегистрТоварыНаСкладах");
        ...
        Для Каждого Вариант Из МассивВариантов Цикл
            Попытка
                Запрос.Текст = СтрШаблон("ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрНакопления.%1", Вариант);
                Запрос.Выполнить();
                // Если дошли сюда - регистр существует
                ТекстЗапроса = СтрШаблон(
                "ВЫБРАТЬ
                |    СУММА(ЕСТЬNULL(%1Остатки.ВНаличииОстаток, 0)) КАК Остаток
                |ИЗ
                |    РегистрНакопления.%1.Остатки(&НаДату, Номенклатура = &Номенклатура И Склад = &Склад) КАК %1Остатки", Вариант);
                Прервать;
            Исключение
                Продолжить;
            КонецПопытки;
