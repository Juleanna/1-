...
Процедура ПосмотретьДвижения(Команда)
    ...
    ТекстДвижений = ПосмотретьДвиженияНаСервере();
    ПоказатьПредупреждение(, ТекстДвижений, , "Движения по документу");
...
Процедура ОстаткиНаСкладе(Команда)
    ...
    ТекстОстатков = ОстаткиНаСкладеНаСервере();
    ПоказатьПредупреждение(, ТекстОстатков, , "Остатки на складах");

КонецПроцедуры

&НаСервере
Функция ПосмотретьДвиженияНаСервере()

    Если НЕ Объект.Проведен Тогда
        Возврат "Документ не проведен. Движений нет.";
    КонецЕсли;

    // Показываем информацию о том, какие движения должны быть
    ТекстДвижений = "";
    ТекстДвижений = ТекстДвижений + "=== ДВИЖЕНИЯ ПО ДОКУМЕНТУ ===" + Символы.ПС;
    ТекстДвижений = ТекстДвижений + "Документ: " + Объект.Ссылка + Символы.ПС;
    ТекстДвижений = ТекстДвижений + "Дата: " + Объект.Дата + Символы.ПС;
    ТекстДвижений = ТекстДвижений + Символы.ПС;

    // Показываем расходы ингредиентов
    ТекстДвижений = ТекстДвижений + "РАСХОДЫ ИНГРЕДИЕНТОВ:" + Символы.ПС;
    ...
    Для Каждого СтрокаБлюда Из Объект.ПриготавливаемыеБлюда Цикл
        Если ЗначениеЗаполнено(СтрокаБлюда.Блюдо) И СтрокаБлюда.КоличествоПорций > 0 Тогда
            ...
            ТекстДвижений = ТекстДвижений + СтрШаблон("Блюдо: %1 (%2 порций)", СтрокаБлюда.Блюдо, СтрокаБлюда.КоличествоПорций) + Символы.ПС;
            ...
            Для Каждого СтрокаИнгредиент Из СоставИнгредиентов Цикл
                Если ЗначениеЗаполнено(СтрокаИнгредиент.Ингредиент) И СтрокаИнгредиент.ОбщееКоличество > 0 Тогда
                    ТекстДвижений = ТекстДвижений + СтрШаблон("  - %1: %2 кг (расход)",
                        СтрокаИнгредиент.Ингредиент,
                        СтрокаИнгредиент.ОбщееКоличество) + Символы.ПС;
                КонецЕсли;
            КонецЦикла;
            ТекстДвижений = ТекстДвижений + Символы.ПС;
        КонецЕсли;
    ...
    // Показываем приход готовых блюд
    ТекстДвижений = ТекстДвижений + "ПРИХОД ГОТОВЫХ БЛЮД:" + Символы.ПС;

    Для Каждого СтрокаБлюда Из Объект.ПриготавливаемыеБлюда Цикл
        Если ЗначениеЗаполнено(СтрокаБлюда.Блюдо) И СтрокаБлюда.КоличествоПорций > 0 Тогда
            ТекстДвижений = ТекстДвижений + СтрШаблон("  + %1: %2 порций (стоимость %3 грн)",
                СтрокаБлюда.Блюдо,
                СтрокаБлюда.КоличествоПорций,
                СтрокаБлюда.ОбщаяПлановаяСебестоимость) + Символы.ПС;
        КонецЕсли;
    КонецЦикла;

    Возврат ТекстДвижений;
...
Процедура РассчитатьОбщуюСебестоимость(ТекущаяСтрока)
    ...
    Если ТекущаяСтрока.КоличествоПорций > 0 И ТекущаяСтрока.ПлановаяСебестоимостьПорции > 0 Тогда
        ТекущаяСтрока.ОбщаяПлановаяСебестоимость = ТекущаяСтрока.КоличествоПорций * ТекущаяСтрока.ПлановаяСебестоимостьПорции;
    Иначе
