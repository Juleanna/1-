...
Процедура ПересчитатьСебестоимость(Команда)
    ...
    Если ЗначениеЗаполнено(Объект.Техкарточка) Тогда
        ПересчитатьСебестоимостьПоТехкарточкеНаСервере();
        Модифицированность = Истина;
        Сообщить("Себестоимость пересчитана: " + Объект.СебестоимостьПорции);
    Иначе
...
&НаСервере
Процедура ПересчитатьСебестоимостьПоТехкарточкеНаСервере()
    
    Если НЕ ЗначениеЗаполнено(Объект.Техкарточка) Тогда
        Возврат;
    КонецЕсли;
    
    // Берем себестоимость напрямую из техкарточки
    Объект.СебестоимостьПорции = Объект.Техкарточка.СебестоимостьПорции;
...
Функция ПолучитьПоследнююЦенуПоступления(Номенклатура)
    ...
    Если ЗначениеЗаполнено(Номенклатура) Тогда
        Попытка
            ...
            Цена = Номенклатура.ЦенаЗакупки;
            Если Цена = 0 Тогда
                // Если цена закупки не указана, используем базовую цену 100 руб/кг для расчета
                Возврат 100;
            КонецЕсли;
            Возврат Цена;
        Исключение
            // Если поле ЦенаЗакупки отсутствует, возвращаем базовую цену
            Возврат 100;
        КонецПопытки;
    Иначе
        Возврат 0;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
    ...
    Если ЗначениеЗаполнено(ТекущийОбъект.Техкарточка) Тогда
        ...
        // Обновляем себестоимость из техкарточки
        ТекущийОбъект.СебестоимостьПорции = ТекущийОбъект.Техкарточка.СебестоимостьПорции;
