...
// Проверяет, есть ли у справочника "Константа" реквизит с указанным именем.
// У объекта справочника нет метода Свойство() — состав реквизитов
// проверяется только по метаданным.
Функция ЕстьРеквизитСправочникаКонстанта(ИмяРеквизита)
    ...
    Возврат МетаСпрКонстанта.Реквизиты.Найти(ИмяРеквизита) <> Неопределено;
...
Функция ПолучитьПользователяODataИзСправочника() Экспорт
    ...
    Попытка
        // Предпочтительно реквизит с именем "Пользователь" (тип: СправочникСсылка.Пользователи)
        Если ЕстьРеквизитСправочникаКонстанта("Пользователь") Тогда
            Возврат Элемент.Пользователь;
        КонецЕсли;

        // Резервный вариант — хранение в ДополнительныхСвойствах объекта
