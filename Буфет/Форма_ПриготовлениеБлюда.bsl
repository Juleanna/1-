...
Процедура ПроверитьНаличиеИнгредиентовНаСервере()
    ...
    ИначеЕсли ФлажокСписанияВМинусУстановлен() Тогда
        Сообщить("ПРОВЕРКА НАЛИЧИЯ ИНГРЕДИЕНТОВ - ЕСТЬ НЕДОСТАТКИ:" + ТекстПроверки
            ...
            |Флажок «Разрешить списание в минус» установлен: документ проведётся,
            |остатки по недостающим ингредиентам уйдут в минус.");
    Иначе
        Сообщить("ПРОВЕРКА НАЛИЧИЯ ИНГРЕДИЕНТОВ - ЕСТЬ НЕДОСТАТКИ:" + ТекстПроверки
            + "

            |Провести документ не получится. Оприходуйте сырьё либо установите
            |флажок «Разрешить списание в минус».");
    КонецЕсли;
