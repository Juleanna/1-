...
Процедура ПриготавливаемыеБлюдаБлюдоПриИзменении(Элемент)
    ...
    Если ЗначениеЗаполнено(ТекущаяСтрока.Блюдо) Тогда
        ПодобратьТехкарточкуСтроки(ТекущаяСтрока);
        ТекущаяСтрока.ПлановаяСебестоимостьПорции = ПолучитьСебестоимостьИзТехкарточки(
            ТекущаяСтрока.Блюдо, ВыбраннаяРецептураСтроки(ТекущаяСтрока));
...
// Выбор рецептуры: список ограничен техкарточками текущего блюда.
//
// Без ограничения в выпадающем списке видны карточки всех блюд сразу,
// а ошибочный выбор ловится только при записи документа.
&НаКлиенте
Процедура ПриготавливаемыеБлюдаТехкарточкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)

    ТекущаяСтрока = Элементы.ПриготавливаемыеБлюда.ТекущиеДанные;
    Если ТекущаяСтрока = Неопределено Или НЕ ЗначениеЗаполнено(ТекущаяСтрока.Блюдо) Тогда
        Возврат;
    КонецЕсли;

    СтандартнаяОбработка = Ложь;

    Отбор = Новый Структура("ГотовоеБлюдо", ТекущаяСтрока.Блюдо);

    ПараметрыФормы = Новый Структура;
    ПараметрыФормы.Вставить("Отбор", Отбор);
    ПараметрыФормы.Вставить("ЗакрыватьПриВыборе", Истина);

    Оповещение = Новый ОписаниеОповещения("ТехкарточкаВыбранаЗавершение", ЭтотОбъект,
        ТекущаяСтрока.ПолучитьИдентификатор());

    ОткрытьФорму("Справочник.Техкарточки.ФормаВыбора", ПараметрыФормы, Элемент, , , , Оповещение);

КонецПроцедуры

&НаКлиенте
Процедура ТехкарточкаВыбранаЗавершение(Результат, ИдентификаторСтроки) Экспорт

    Если Результат = Неопределено Тогда
        Возврат;
    КонецЕсли;

    СтрокаБлюда = Объект.ПриготавливаемыеБлюда.НайтиПоИдентификатору(ИдентификаторСтроки);
    Если СтрокаБлюда = Неопределено Тогда
        Возврат;
    КонецЕсли;

    СтрокаБлюда["Техкарточка"] = Результат;

    СтрокаБлюда.ПлановаяСебестоимостьПорции = ПолучитьСебестоимостьИзТехкарточки(
        СтрокаБлюда.Блюдо, Результат);
    РассчитатьОбщуюСебестоимость(СтрокаБлюда);
    ОбновитьЗаголовокФормы();
...
Процедура ЗаполнитьПоТехкарточкамНаСервере()
    ...
    Попытка
        Запрос = Новый Запрос;
        // Строка добавляется на каждую техкарточку, а не на блюдо: у блюда
        // с двумя рецептурами это две разные строки, и по какой готовить —
        // видно сразу
        Запрос.Текст =
        "ВЫБРАТЬ
    ...
    Пока Выборка.Следующий() Цикл
        ...
        Попытка
            НоваяСтрока["Техкарточка"] = Выборка.Техкарточка;
        Исключение
            // Реквизита ещё нет — состав возьмётся по первой карточке блюда
        КонецПопытки;
    КонецЦикла;
...
Процедура ПроверитьНаличиеИнгредиентовНаСервере()
    ...
    //
    // Момент расчёта тот же, что и при проведении: для документа задним
    // числом — текущий, иначе кнопка показывала бы «всё в порядке»,
    // а проведение отказывало.
    МоментПроверки = Объект.Дата;
    Если ЗначениеЗаполнено(Объект.Дата) И Объект.Дата < ТекущаяДатаСеанса() Тогда
        МоментПроверки = Неопределено;
    КонецЕсли;

    Результат = РасчетСебестоимостиМодуль.ПроверитьНаличиеИнгредиентов(
        Объект.ПриготавливаемыеБлюда, Объект.СкладСырья, МоментПроверки);
...
// Техкарточка строки: реквизит табличной части может быть ещё не создан.
&НаКлиенте
Функция ВыбраннаяРецептураСтроки(СтрокаБлюда)
...
// Подставляет рецептуру, если у блюда она единственная.
//
// Когда карточек несколько, поле остаётся пустым: выбор делает повар.
// Пустое поле — не ошибка, тогда берётся первая карточка по наименованию.
&НаКлиенте
Процедура ПодобратьТехкарточкуСтроки(СтрокаБлюда)

    Попытка
        СтрокаБлюда["Техкарточка"] = ПодобратьЕдинственнуюТехкарточку(СтрокаБлюда.Блюдо);
    Исключение
        // Реквизита ещё нет — состав возьмётся по первой карточке блюда
    КонецПопытки;
...
Функция ПодобратьЕдинственнуюТехкарточку(Блюдо)

    Возврат РасчетСебестоимостиМодуль.ЕдинственнаяТехкарточкаБлюда(Блюдо);

КонецФункции

&НаСервереБезКонтекста
Функция ПолучитьСебестоимостьИзТехкарточки(Блюдо, Техкарточка = Неопределено)

    Если НЕ ЗначениеЗаполнено(Блюдо) И НЕ ЗначениеЗаполнено(Техкарточка) Тогда
        Возврат 0;
    КонецЕсли;
    ...
    //
    // Если рецептура выбрана, берётся её себестоимость: у блюда с несколькими
    // карточками они разные, и подставлять чужую нельзя.
    Попытка
        Запрос = Новый Запрос;
