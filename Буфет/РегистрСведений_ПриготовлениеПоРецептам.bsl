// Модуль менеджера регистра сведений "Приготовление по рецептам"
// Для учета фактического приготовления блюд и списания ингредиентов
//
// Аналитические функции размещены в модуле МЕНЕДЖЕРА и объявлены с Экспорт,
// чтобы их можно было вызывать как РегистрыСведений.ПриготовлениеПоРецептам.<Функция>().
//
// Обработчики событий записи (ПередЗаписью, ПриЗаписи) живут в модуле
// НАБОРА ЗАПИСЕЙ и имеют сигнатуру (Отказ, Замещение). События с именем
// "ОбработкаЗаписи" в платформе нет — такой обработчик никогда не вызывается.

#Область ПрограммныйИнтерфейс

// Получить данные приготовления за период
Функция ПолучитьДанныеПриготовленияЗаПериод(НачалоПериода, КонецПериода, Рецепт = Неопределено, ГотовоеБлюдо = Неопределено) Экспорт
...
Функция ПолучитьСтатистикуРасходаИнгредиентов(НачалоПериода, КонецПериода, Ингредиент = Неопределено) Экспорт
...
Функция ПолучитьСтатистикуПриготовленныхБлюд(НачалоПериода, КонецПериода, ГотовоеБлюдо = Неопределено) Экспорт
...
Функция ПолучитьСтатистикуПоДням(НачалоПериода, КонецПериода, ГотовоеБлюдо = Неопределено) Экспорт
...
Функция ПроверитьСоответствиеПлановомуРасходу(НачалоПериода, КонецПериода, ГотовоеБлюдо = Неопределено) Экспорт
